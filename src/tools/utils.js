...
const getHttpSalt = (timestamp) => {
  ...
  const salt = crypto.createHash('md5').update(string).digest('hex');
  return salt
}
...
const getPageStartAndEnd = (page, size) => {
  if (!page || !size) {
    throw new Error('getPageStartAndEnd paramters can not be null')
  }
...
const getPageOffsetAndLimit = (page, size) => {
  if (!page || !size) {
    throw new Error('getPageStartAndEnd paramters can not be null')
  }
